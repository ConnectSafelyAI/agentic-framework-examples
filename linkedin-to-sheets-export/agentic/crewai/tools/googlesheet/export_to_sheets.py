...
def export_to_sheets(
    ...
    try:
        ...
        return {
            ...
            "summary": f"{'Created' if is_new_sheet else 'Updated'} spreadsheet with {people_added} new people" + (f" ({people_skipped} duplicates skipped)" if people_skipped > 0 else ""),
        }
    except (ValueError, Exception) as e:
        return {"success": False, "error": str(e) if isinstance(e, ValueError) else f"Export failed: {str(e)}"}
