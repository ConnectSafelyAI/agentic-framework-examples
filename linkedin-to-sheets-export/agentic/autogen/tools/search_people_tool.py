...
def search_people(
    ...
    try:
        response = requests.post(
            ...
            json=payload,
            timeout=60,  # Longer timeout for large searches
        )
        ...
        # Handle array wrapper - API may return array with single object
...
def _extract_company(headline: str) -> str:
    """Extract company name from LinkedIn headline (e.g., 'CEO at Company Name')."""
    if not headline:
        return ""

    import re
    # Match patterns like "at Company", "@ Company", "- Company"
